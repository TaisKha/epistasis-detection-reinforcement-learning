...
class EpisodeEvents(EventEnum):
    ...
    MAX_ITERATIONS_REACHED = "max_iteration_reached"
...
class EndOfEpisodeHandler:
    def __init__(self, exp_source: ptan.experience.ExperienceSource, alpha: float = 0.98,
                 ...
                 subsample_end_of_episode: Optional[int] = None, max_iter:Optional[int] = None):
        """
    ...
    def attach(self, engine: Engine):
        ...
        State.event_to_attr[EpisodeEvents.MAX_ITERATIONS_REACHED] = "episode"
    ...
    def __call__(self, engine: Engine):
        for reward, steps in self._exp_source.pop_rewards_steps():
            ...
            if self._max_iter is not None and engine.state.metrics['episode_number'] > self._max_iter:
                engine.fire_event(EpisodeEvents.MAX_ITERATIONS_REACHED)
            if engine.state.metrics['reward'] is not None and engine.state.metrics['reward'] >= self._bound_avg_reward:
...
def setup_ignite(engine: Engine, params: SimpleNamespace,
    ...
    handler = EndOfEpisodeHandler(
        exp_source, bound_avg_reward=params.stop_reward, max_iter=params.max_iter)
    ...
    def game_solved(trainer: Engine):
        f = open('result3dqn.txt', 'a')    
        passed = trainer.state.metrics['time_passed']
        f.write("Game solved in %s, after %d episodes "
              "and %d iterations! \n" % (
            timedelta(seconds=int(passed)),
    ...
    @engine.on(EpisodeEvents.MAX_ITERATIONS_REACHED)
    def gameNotSolved(trainer: Engine):
        f = open(LOG_FILE, 'a')    
        passed = trainer.state.metrics['time_passed']
        f.write("Failure %s, after %d episodes "
              "and %d iterations! \n" % (
            timedelta(seconds=int(passed)),
            trainer.state.episode, trainer.state.iteration))
        f.close()
    ...
    wandb_logger = WandBLogger(
        ...
        entity="taisikus",
        config={
        'replay_initial': params.replay_initial,
        'target_net_sync': params.target_net_sync,
        'epsilon_frames': params. epsilon_frames,
